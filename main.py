...
import itertools
from my_life import keep_alive
...
description = "Use V.cmd_name To Use Butler"
...
status = cycle(["Masters Orders!", "v.help", "She Knows", "Dead Or Alive", "Awakening", "Call Thy Name"])
...
async def on_command_error(msg, error):
  if isinstance(error, commands.CommandNotFound):
    await msg.send("No such command exists!")

#@bot.command(description = "Replies Hi!\nSyntax: V.hi")
#async def hi(msg):
#  await msg.send("hiiii")

@bot.command(description = "Changes The Prefix Of The Bot For Your Server!")
async def change_prefix(msg, prefix):
  with open('prefixes.json', 'r') as f:
    prefixes = json.load(f)
  prefixes[str(msg.guild.id)] = prefix

  with open('prefixes.json', 'w') as f:
    json.dump(prefixes, f, indent = 4)
  await msg.send(f'Prefix changed to {prefixes[1]}')

@bot.command(description = "Tells The Name Of Butler's Master!\nSyntax: V.master")
async def master(msg):
  await msg.send(os.environ['master'])

@bot.command(description = "Repeats Whatever You Type!\nSyntax: V.repeat (1-20) content")
async def repeat(msg, times: int, content='repeating...'):
  if times>20:
    await msg.send("Max range is 20!")
  else:
    for i in range(times):
        await msg.send(content)

@repeat.error
async def repeat_error(msg, error):
  if isinstance(error, commands.MissingRequiredArgumnets):
    await msg.send("All The Required Arguments Were Not Passed!\Syntax: V.repeat (1-20) content")

@bot.command(description = "Clears The Above 'n' Messages!\nSyntax: V.clear (1-100)")
@commands.has_permissions(manage_messages = True)
async def clear(msg, amount = 6):
    await msg.channel.purge(limit = (amount+1))

@bot.command(description = "Kicks The Specified Member with/without reason!\nSyntax: V.kick @user reason")
@commands.has_permissions(kick_members = True)
async def kick(msg, member: discord.Member, *, reason = None):
    await member.kick(reason = reason)
    await msg.send(f'{msg.author.mention} Kicked {member.mention}')

@bot.command(description = "Banss The Specified Member with/without reason!\nSyntax: V.ban @user reason")
@commands.has_permissions(ban_members = True)
async def ban(msg, member: discord.Member, *, reason = None):
    await member.ban(reason = reason)
    await msg.send(f'{msg.author.mention} Banned {member.mention}')

@bot.command(description = "Forgives The Specified Member!\nSyntax: V.pardon username#number")
@commands.has_permissions(administrator = True)
async def pardon(msg, member):
    banned_users = await msg.guild.bans()
    m_name, m_code = member.split('#')

    for ban_entry in banned_users:
      user = ban_entry.user
      if(user.name , user.discriminator) == (m_name, m_code):
        await msg.guild.unban(user)
        await msg.send(f'{msg.author.mention} Forgave {user.name}#{user.discriminator}')

@bot.command(description = "Shows All The Banned Members!\nSyntax: V.banned")
@commands.has_permissions(administrator = True)
...
@bot.command(description = "Prints details of Server")
@commands.has_permissions(administrator = True)
async def about_serv(ctx):
    ...
    async for member in ctx.guild.fetch_members(limit=150) :
        await ctx.send('Name : {}\t Status : {}\nJoined on {}'.format(member.display_name, str(member.status.name), str(member.joined_at)))

@bot.command()
async def about_bot(ctx):
    await ctx.send("I Am Butler Of Dr_OneEyed!\nAt present I have limited features(find out more by typing v.help)")
    try:
        user = await bot.fetch_user(os.environ['master'])
        await ctx.send(f'Owner: {user.mention}')
    except:
        user = os.environ['master']
        await ctx.send(f'Owner: **{user}**')
    async for member in ctx.guild.fetch_members(limit=150) :
        if member.display_name == "Butler":
          await ctx.send('Name : {}\nJoined on {}'.format(member.display_name,str(member.joined_at)))
